...
public class solutionsScreen extends JFrame implements KeyListener {
    ...
    public void View_solution(int solutions_counter) {
        frame.setTitle("N Queens solver - " + "Solution NO. " + (solutions_counter+1) + "/" + ThreadSolver.solutions.size() + " Thread NO. " + ThreadSolver.solutions.get(solutions_counter).TN);
        int counter = 0;
        boolean Queen;
        int col = 0;
        int row = 0;
        if (n % 2 != 0) {
            for (int i = 0; i < n * n; i++) {
                ...
                if (ThreadSolver.solutions.get(solutions_counter).board[row][col] == true) {
                    Queen = true;
    ...
    public void keyPressed(KeyEvent e) {
        if (e.getKeyCode() == KeyEvent.VK_RIGHT && this.solutions_counter != ThreadSolver.solutions.size()-1) {
